...
var ShortestPathUtils = {
    ...
    makeSubmitInput: function (value) {
        ...
        input = ShortestPathUtils.addElementProperty(input, "onclick", function () {
            ShortestPathUtils.callDrawGraph();
        });
        return input;
    },
    ...
    callDrawGraph: function () {
        ...
        for (var i = 0; i < distances.length; i++) {
            if (distances[i].value != '' && parseInt(distances[i].value) && distances[i].value != 0) {
                ...
                paths.push({
                    source: parseInt(fields[0]),
                    target: parseInt(fields[1]),
                    distance: parseInt(distances[i].value)
                });
